...
var dag_pb_esm_src_namespaceObject = {};
__nccwpck_require__.r(dag_pb_esm_src_namespaceObject);
__nccwpck_require__.d(dag_pb_esm_src_namespaceObject, {
  "code": () => (src_code),
...
__nccwpck_require__.r(errors_namespaceObject);
__nccwpck_require__.d(errors_namespaceObject, {
  "abortedError": () => (abortedError),
  "notFoundError": () => (notFoundError)
});

// NAMESPACE OBJECT: ./node_modules/carbites/node_modules/@ipld/dag-cbor/esm/index.js
var dag_cbor_esm_namespaceObject = {};
__nccwpck_require__.r(dag_cbor_esm_namespaceObject);
__nccwpck_require__.d(dag_cbor_esm_namespaceObject, {
  "code": () => (esm_code),
...
var p_retry = __nccwpck_require__(2548);
// EXTERNAL MODULE: ./node_modules/it-last/index.js
var it_last = __nccwpck_require__(7123);
// EXTERNAL MODULE: ./node_modules/it-pipe/index.js
var it_pipe = __nccwpck_require__(7185);
...
var it_parallel_batch = __nccwpck_require__(6615);
// EXTERNAL MODULE: ./node_modules/merge-options/index.js
var merge_options = __nccwpck_require__(2555);
...
var err_code = __nccwpck_require__(2997);
...
const persist = async (buffer, blockstore, options) => {
  if (!options.codec) {
    options.codec = dag_pb_esm_src_namespaceObject;
  }
  if (!options.hasher) {
  ...
  if (options.codec === dag_pb_esm_src_namespaceObject && options.hasher !== sha256) {
    options.cidVersion = 1;
  }
...
var it_all = __nccwpck_require__(5810);
...
var it_batch = __nccwpck_require__(3454);
...
async function* bufferImporter(file, block, options) {
  for await (let buffer of file.content) {
    yield async () => {
      ...
      let unixfs;
      const opts = {
        codec: dag_pb_esm_src_namespaceObject,
...
var rabin_wasm_src = __nccwpck_require__(1715);
...
var hamt_sharding_src = __nccwpck_require__(7820);
...
var browser_readablestream_to_it = __nccwpck_require__(664);
// EXTERNAL MODULE: ./node_modules/blob-to-it/index.js
var blob_to_it = __nccwpck_require__(7842);
// EXTERNAL MODULE: ./node_modules/it-peekable/index.js
var it_peekable = __nccwpck_require__(2276);
// EXTERNAL MODULE: ./node_modules/it-map/index.js
var it_map = __nccwpck_require__(8753);
...
var it_drain = __nccwpck_require__(7351);
// EXTERNAL MODULE: ./node_modules/it-filter/index.js
var it_filter = __nccwpck_require__(2220);
// EXTERNAL MODULE: ./node_modules/it-take/index.js
var it_take = __nccwpck_require__(2360);
...
;// CONCATENATED MODULE: ./node_modules/@web3-storage/parse-link-header/index.js
...
const dag_cbor_resolve = async (cid, name, path, toResolve, resolve, depth, blockstore, options) => {
  ...
  while (toResolve.length) {
    ...
    if (prop in subObject) {
      ...
      const subObjectCid = cid_CID.asCID(subObject[prop]);
      if (subObjectCid) {
        return {
          entry: {
            type: 'object',
...
const identity_resolve = async (cid, name, path, toResolve, resolve, depth, blockstore, options) => {
  ...
  const buf = await digest_decode(cid.multihash.bytes);
  return {
    entry: {
      type: 'identity',
      name,
...
const resolvers = {
  [src_code]: unixfs_v1,
  [raw_code]: resolvers_raw,
  [code]: dag_cbor,
  [identity_identity.code]: resolvers_identity
};
...
const cidAndRest = path => {
  if (path instanceof Uint8Array) {
    return {
      cid: cid_CID.decode(path),
      toResolve: []
    };
  }
  const cid = cid_CID.asCID(path);
  if (cid) {
    return {
      cid,
      toResolve: []
...
class verifying_get_only_blockstore_VerifyingGetOnlyBlockStore extends base_BaseBlockstore {
...
async function isValid({ cid, bytes }) {
    ...
    return equals_equals(hash.digest, cid.multihash.digest);
}
...
;// CONCATENATED MODULE: ./node_modules/carbites/node_modules/@ipld/dag-cbor/esm/index.js
...
class TreewalkCarSplitter {
  constructor(reader, targetSize, options = {}) {
    ...
    this._decoders = [
      dag_pb_esm_src_namespaceObject,
      raw_namespaceObject,
...
// EXTERNAL MODULE: ./node_modules/it-glob/index.js
var it_glob = __nccwpck_require__(402);
...
const fsStat = (0,external_util_.promisify)(graceful_fs.stat);
...
async function* filesFromPath(paths, options) {
  ...
  for await (const path of paths) {
    if (typeof path !== 'string') {
      throw err_code(new Error('Path must be a string'), 'ERR_INVALID_PATH', { path });
    }
    const absolutePath = external_path_.resolve(process.cwd(), path);
    const stat = await fsStat(absolutePath);
    const prefix = options.pathPrefix || external_path_.dirname(absolutePath);
...
async function* toGlobSource({path, type, prefix, mode, mtime, size, preserveMode, preserveMtime}, options) {
  ...
  if (type === 'file') {
    yield {
      ...
      stream: () => graceful_fs.createReadStream(external_path_.isAbsolute(path) ? path : external_path_.join(process.cwd(), path)),
      mode,
      mtime,
      size
    };
  ...
  for await (const p of it_glob(path, '**/*', globOptions)) {
    ...
    yield {
      ...
      stream: () => graceful_fs.createReadStream(p),
      mode,
      mtime,
      size: stat.size
    };
...
var dist_src = __nccwpck_require__(2371);
...
;// CONCATENATED MODULE: ./node_modules/@web-std/fetch/src/index.js
/**
 * Index.js
 *
			...
			if (isRedirect(Number(response_.statusCode))) {
				...
				switch (request.redirect) {
					...
					case 'follow': {
						...
						src_fetch(new Request(locationURL.href, requestOptions)).then(resolve, reject);
						finalize();
						return;
					}
			...
			if (codings === 'deflate' || codings === 'x-deflate') {
				...
				raw.once('data', chunk => {
					...
					if ((chunk[0] & 0x0F) === 0x08) {
						body = (0,external_stream_.pipeline)(body, external_zlib_namespaceObject.createInflate(), reject);
					} else {
						body = (0,external_stream_.pipeline)(body, external_zlib_namespaceObject.createInflateRaw(), reject);
					}
 ...
 */
class file_File extends lib_node_Blob {
  /**
   *
   * @param {BlobPart[]} init
...
const lib_node_File = typeof globalThis.File === "function" ? globalThis.File : file_File
...
;// CONCATENATED MODULE: ./node_modules/web3.storage/src/platform.js
...
class Web3Storage {
   ...
   */
  static async put ({ endpoint, token, rateLimiter = globalRateLimiter, fetch = src_fetch }, files, {
    onRootCidReady,
    onStoredChunk,
   ...
   * @returns {Promise<CIDString>}
   */
  static async putCar ({ endpoint, token, rateLimiter = globalRateLimiter, fetch = src_fetch }, car, {
    name,
    onStoredChunk,
    maxRetries = MAX_PUT_RETRIES,
    maxChunkSize = DEFAULT_CHUNK_SIZE,
    ...
    const onCarChunk = async car => {
      ...
      const carFile = new lib_node_Blob(carParts, { type: 'application/vnd.ipld.car' })
   ...
   */
  static async get ({ endpoint, token, rateLimiter = globalRateLimiter, fetch = src_fetch }, cid, options = {}) {
   ...
   */
  static async status ({ endpoint, token, rateLimiter = globalRateLimiter, fetch = src_fetch }, cid, options = {}) {
  ...
  static async * list (service, { before = new Date().toISOString(), maxResults = Infinity, signal } = {}) {
     ...
     */
    async function listPage ({ endpoint, token, rateLimiter = globalRateLimiter, fetch = src_fetch }, { before, size }) {
...
async function toWeb3File ({ content, path, cid }) {
  ...
  const file = new lib_node_File(chunks, toFilenameWithPath(path))
...
// please dont steal
const API_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJkaWQ6ZXRocjoweGE4MkJCMTcxODFFQmMyZTMxMUQ2ZGU1MjA1RTNDYzg0NzdlNThBNUQiLCJpc3MiOiJ3ZWIzLXN0b3JhZ2UiLCJpYXQiOjE2NjE5NjEzNDU0NTUsIm5hbWUiOiJnaXQtYmFja3VwLXRlc3QifQ.Qhk_dWL8kKDuUSbAtjkdEa_ZZeTTZv-eqJXBBwcACNk';
const client = new Web3Storage({ token: API_TOKEN });
